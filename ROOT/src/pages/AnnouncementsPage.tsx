    ...
    <div className="space-y-8">
      ...
      <div className="space-y-6">
        {filtered.map((ann) => (
          <div
            ...
            className={`p-6 sm:p-8 rounded-3xl bg-[#0E4A87]/70 border transition-all cursor-pointer group space-y-4 ${
              ann.pinned ? 'border-[#1976D2]/50 shadow-[0_0_20px_rgba(25,118,210,0.15)]' : 'border-white/20 hover:border-zinc-700'
            }`}
          >
            ...
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                ...
                <span className="px-2.5 py-1 rounded-lg bg-[#1976D2]/20 text-sky-200 border border-[#1976D2]/30 text-xs font-semibold">
                  {ann.category}
                </span>
              </div>

              <div className="flex items-center gap-3 text-xs font-mono text-sky-200">
            ...
            <div className="space-y-2">
              <h2 className="text-xl sm:text-2xl font-extrabold heading-gold group-hover:text-sky-200 transition-colors">
                {ann.title}
              </h2>
            ...
            <div className="pt-4 border-t border-white/20 flex flex-wrap items-center justify-between gap-3 text-xs">
              <div className="flex flex-wrap items-center gap-1.5">
                {ann.tags?.map((t) => (
                  <span key={t} className="px-2 py-0.5 rounded bg-[#0B3C70]/55 text-sky-200 border border-white/20 font-mono text-[11px]">
                    #{t}
                  </span>
                ))}
              </div>
              ...
              <div className="flex items-center gap-2 text-sky-200">
                ...
                <span className="text-sky-300 font-bold ml-2 group-hover:translate-x-1 transition-transform">Read full announcement &rarr;</span>
              </div>
            </div>
          </div>
        ))}
      ...
      {activeModalAnn && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fadeIn">
          <div
            className="w-full max-w-3xl bg-[#0E4A87]/70 border border-[#1976D2]/40 rounded-3xl p-6 sm:p-8 shadow-[0_25px_60px_rgba(0,0,0,0.9)] max-h-[85vh] overflow-y-auto space-y-6 relative"
            onClick={(e) => e.stopPropagation()}
          >
            <button
              onClick={() => setActiveModalAnn(null)}
              className="absolute top-6 right-6 p-2 rounded-xl bg-[#0B3C70]/55 text-sky-200 hover:text-white border border-white/20"
            >
              <X size={18} />
            </button>
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                <span className="px-3 py-1 rounded-full bg-[#1976D2]/20 text-sky-200 border border-[#1976D2]/30 text-xs font-semibold">
                  {activeModalAnn.category}
                </span>
                <span className="text-xs text-sky-200 font-mono">{activeModalAnn.date}</span>
              </div>
              <h2 className="text-2xl sm:text-3xl font-extrabold heading-gold">
            ...
            <div className="prose prose-invert max-w-none text-sm text-sky-100 leading-relaxed space-y-4 whitespace-pre-line border-t border-b border-white/20 py-6">
            ...
            <div className="flex items-center justify-between">
              <div className="flex flex-wrap gap-1.5">
                {activeModalAnn.tags?.map((t) => (
                  <span key={t} className="px-2 py-0.5 rounded bg-[#0B3C70]/55 text-sky-200 text-xs font-mono">
                    #{t}
                  </span>
                ))}
              </div>
              <button
                onClick={() => setActiveModalAnn(null)}
                className="px-5 py-2 rounded-xl bg-[#1976D2] hover:bg-[#42A5F5] text-white text-xs font-bold"
              >
                Close Article
              </button>
            </div>
